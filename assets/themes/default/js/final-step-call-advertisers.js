...
function checkForCompletion() {
	$.ajax('https://network4rentals.com/network/ajax/checkForTermsAdvertisers', {
		success: function(response) {
			if(response.indexOf('1') > -1) {
				$('.fadeout').hide();
				$('.removeOnSuccess').remove();
				$('.waiting').html('<i class="fa fa-exclamation text-success"></i> Finished');
				$('.agreed').html('Yes').removeClass('label-danger');
...
function resendEmailClicked() {
	$.ajax('https://network4rentals.com/network/advertisers/resend-email', {
		success: function(response) {
