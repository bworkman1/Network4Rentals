<div class="row">
	<div class="col-sm-8">
		<h3><i class="fa fa-bar-chart fa-fw text-primary"></i> My Premium Stats</h3>
	</div>
	<div class="col-sm-4">
		<br>
		<?php
			if(!empty($my_zips)) {
				...
				echo form_open('local-partner/stats');
...
<?php if($page_setting) { // Show Ad Stats ?>
	<?php if(!empty($my_zips)) { // Show Ad Stats ?>
		...
		<?php if(isset($_POST['select_ad'])) { ?>
			<a href="<?php echo base_url('local-partner/edit-post/'.$my_zips[$_POST['select_ad']]->id); ?>" class="btn btn-primary btn-sm pull-right"><i class="fa fa-pencil"></i> Edit This Post</a>
		<?php } else { ?>
			<a href="<?php echo base_url('local-partner/edit-post/'.$my_zips[0]->id); ?>" class="btn btn-primary btn-sm pull-right"><i class="fa fa-pencil"></i> Edit This Post</a>
		<?php } ?>
	<?php } else { ?>
		<p>You don't have any active subscriptions, running right now. Once you add a subscription to our services you will be able to see stats about your ads and how many page visitors you have in real time.</p><a href="<?php echo base_url('local-partner/add-zip-codes'); ?>" style="margin-top: 5px" class="btn btn-primary btn-sm"><i class="fa fa-map-marker"></i> Add Zips/Services</a>
	<?php } ?>
</div>
