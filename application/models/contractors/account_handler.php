<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
    ...
    class Account_handler extends CI_Model {
		...
		{
			$results = $this->db->get_where('payments', array('user_id'=>$this->session->userdata('user_id'), 'type'=>'contractor', 'active'=>'y')); //Will need updated - might return multiple rows
			if($results->num_rows()>0) {
				return $results->row();
			} else {
				return false;
		...
		public function update_password($password) 
		{
			...
			$results = $this->db->update('contractors', array('password'=> $password));
