    ...
    class Create_account_handler extends CI_Model {
		...
		public function account_builder($data)
		{
			...
			*/
			...
			if($result) {
				$result = $this->submit_payment($data);
		...
		public function submit_payment($data) //**********FIRST RETURN WILL NEED TO BE REMOVED TO ACTUALLY PROCESS PAYMENTS
		{			
			/*
		...
		function add_zips_to_account($contractor_id, $sub_id)
		{
			...
			for($i=0;$i<count($zips);$i++) {
				$this->db->insert('contractor_zip_codes', array('zip'=>$zips[$i], 'contractor_id'=>$service[$i], 'service_type'=>$service[$i], 'sub_id'=>$sub_id));
