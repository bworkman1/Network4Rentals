<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	class reset_password extends CI_Model {
		...
		function set_email_hash($user) // Updates database with a hash of the users username
		{
			...
			$sql = "UPDATE landlords SET loginHash = ? WHERE user = ? AND confirmed = 'y' LIMIT 1";
		...
		{
			$query_str = "SELECT user FROM landlords WHERE email = ? AND confirmed = 'y' LIMIT 1";
		...
		{
			$query_str = "SELECT id FROM landlords WHERE loginHash = ? AND confirmed = 'y' LIMIT 1";
		...
		{
			$query_str = "UPDATE landlords SET pwd = ?, loginHash = ? WHERE loginHash = ? AND confirmed = 'y' LIMIT 1";
