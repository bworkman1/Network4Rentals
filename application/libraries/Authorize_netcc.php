 ...
 */
class Authorize_netcc
{
    private $CI;						// CodeIgniter instance

    public $api_login_id = '6DEa37j5G594';			// API Login ID
    public $api_transaction_key = '383dhhU6649FJvZU';	// API Transaction Key
    public $api_url = 'https://test.authorize.net/gateway/transact.dll';				// Where we postin' to?
	...
	public function __construct( $config = array() )
	{
		...
		// Load config file
		$this->CI->config->load('authorize_netcc', TRUE);
		
		foreach( $this->CI->config->item('authorize_netcc') as $key => $value )
		{
			if( isset($this->$key) )
			{
				$this->$key = $value;
