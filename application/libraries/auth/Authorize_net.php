...
class Authorize_net
{
    ...
    public $api_login_id = '';			// API Login ID
    public $api_transaction_key = '';	// API Transaction Key
	...
	{
		if(!empty($this->newApi)) {
			$id = $this->newApi;
		} else {
			$id = $this->api_login_id;
		}
		if(!empty($this->newKey)) {
			$key = $this->newKey;
		} else {
			$key = $this->api_transaction_key;
		}
		...
		elseif( is_string($response) && strpos($response, '|') !== FALSE )
		{
			...
			if( isset($res[0]) )
			{
				switch( $res[0] )
				{
					...
					if( isset($res[3]) )
					{
						$this->error = $res[3]; //.' = '.$this->api_login_id.' = '.$this->api_transaction_key;
					}
					return FALSE;
					break;
